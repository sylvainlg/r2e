 ...
 */
final class Version20191002180039 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, email, roles, password, group_error_mail FROM user');
        ...
        , password VARCHAR(255) NOT NULL COLLATE BINARY, group_error_mail BOOLEAN NOT NULL DEFAULT 0, send_email_on_error BOOLEAN NOT NULL DEFAULT 1)');
        $this->addSql('INSERT INTO user (id, email, roles, password, group_error_mail) SELECT id, email, roles, password, group_error_mail FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        , password VARCHAR(255) NOT NULL, group_error_mail BOOLEAN DEFAULT \'0\' NOT NULL)');
        $this->addSql('INSERT INTO user (id, email, roles, password, group_error_mail) SELECT id, email, roles, password, group_error_mail FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8D93D649E7927C74 ON user (email)');
    }
